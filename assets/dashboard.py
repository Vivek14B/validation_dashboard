...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                # UPDATED: users table with all new columns
                cursor.execute(f'''
                    CREATE TABLE IF NOT EXISTS `users` (
                        `id` INT PRIMARY KEY AUTO_INCREMENT, 
                        `username` VARCHAR(255) UNIQUE NOT NULL, `full_name` VARCHAR(255),
                        `email` VARCHAR(255), `mobile_number` VARCHAR(20),
                        `hashed_password` TEXT NOT NULL, `role` VARCHAR(255) NOT NULL, 
                        `reports_to` TEXT, `mapped_to_management` TEXT,
                        `can_upload` BOOLEAN DEFAULT NULL, `disabled_pages` TEXT
                    ) {table_options}''')

                # NEW: Table to store permissions for each role
                cursor.execute(f'''
                ...
                # NEW: Table for correction status escalations
                cursor.execute(f'''
                ...
                # NEW: Table for clarification waivers
                cursor.execute(f'''
                    CREATE TABLE IF NOT EXISTS `clarification_waivers` (
                        `id` INT PRIMARY KEY AUTO_INCREMENT,
                        `username` VARCHAR(255) UNIQUE NOT NULL,
                        `waived_until` DATE NOT NULL,
                        `waived_by` VARCHAR(255) NOT NULL,
                        `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP
                    ) {table_options}''')

                # NEW: Table for the notification system
                cursor.execute(f'''
                ...
                # Populate default permissions for roles if they don't exist
    ...
    def get_user(self, username):
        ...
        try:
            with conn.cursor(dictionary=True) as cursor:
                cursor.execute("SELECT username, full_name, hashed_password, role FROM `users` WHERE username = %s", (username,)); return cursor.fetchone()
        except mysql.connector.Error as err:
    ...
    def get_all_users(self):
        conn = self._get_connection()
        try:
            query = "SELECT id, username, full_name, email, mobile_number, role, reports_to, mapped_to_management, can_upload, disabled_pages FROM `users`"
...
def main():
    ...
    # UPDATED: Added "Clarification Center" to the lists for relevant roles
    page_access = {
        "Super User": [
            ...
            if submitted:
                if not clarification_text.strip():
                    ...
                    if success:
                        ...
                        db_manager.create_notification(username='Super User', notif_type='Clarification Submitted', message=f"User '{username}' submitted a clarification for runs: {run_ids_str}.")
                        # You could add more complex logic here to notify the user's specific manager/management
                        st.session_state['clarification_required'] = False
    ...
    start_date, end_date = None, None
    pages_with_filter = ["📊 Dashboard Analytics", "📈 Trends & History", "📋 Exception Details", "📍 Location Expenses", "🧾 Ledger/Sub-Ledger Summary", "👤🧾 User-wise Ledger Exceptions", "👤📍 User & Location Analysis", "👤📊 User Performance", "📝 Correction Status"]
    ...
    # Page Routing now includes the new pages
