...
def check_password():
    ...
    def validate_login(username, password):
        ...
        if user_data:
            # user_data is a tuple: (username, hashed_password, role)
            ...
            if bcrypt.checkpw(password.encode('utf-8'), stored_hashed_password):
                ...
                if user_data[2] == "Manager":
                    st.session_state["managed_users"] = db_manager.get_managed_users(user_data[0])
                return True
...
def show_report_archive_page():
    ...
    # Get user context from session state to enforce permissions
    user_role = st.session_state.get("role")
    username = st.session_state.get("username_actual")
    managed_users = st.session_state.get("managed_users", [])

    # Fetch history filtered by the user's specific role and scope
    history_df = db_manager.get_validation_history(
        user_role=user_role,
        username=username,
        managed_users=managed_users
    ...
    if history_df.empty:
        if user_role == "User":
            st.warning("No validation reports found for your user account.")
        elif user_role == "Manager":
            st.warning("No validation reports found for the users you manage.")
        else: # Management
    ...
    if selected_run_display:
        ...
        # Perform security check with all necessary context before allowing download
        report_data, filename = db_manager.get_archived_report(
            selected_run_id, 
            user_role=user_role, 
            username=username, 
            managed_users=managed_users
        ...
        if report_data:
            ...
            st.error("No archived report found for this run, or you do not have permission to access it.")

    # The aggregate download feature is only shown to roles that can see more than their own data.
    # It operates on the already-filtered history_df, so it is inherently secure.
    if user_role in ["Manager", "Management"]:
        ...
        if st.button("Generate & Download Aggregate Report", key="agg_report_btn"):
            if not selected_multi_runs:
                st.warning("Please select at least one run to aggregate.")
            else:
...
def main():
    ...
    # This is the security gate for the entire application.
    # If the user is not logged in, the script stops here.
    if not check_password():
        st.stop()

    # 2. GET USER CONTEXT FROM SESSION STATE
    # This information is available because the user has successfully logged in.
    user_role = st.session_state.get("role")
    username = st.session_state.get("username_actual")
    managed_users = st.session_state.get("managed_users", [])
    ...
    # 4. ROLE-BASED ACCESS CONTROL (RBAC) FOR NAVIGATION
    ...
    page_access = {
        ...
        "Management": [
            ...
            "🗑️ Data Management",
            "⚙️ Settings",
            "🛠️ User Management"  # The new page for Management
        ]
    }
    ...
    # Display the navigation radio buttons in the sidebar
    with st.sidebar:
        selected_page = st.radio("Main Navigation:", page_navigation_options, label_visibility="collapsed")

    # This subtitle appears below the main title on every page
    ...
    # 5. CENTRALIZED DATE FILTER LOGIC
    start_date, end_date = None, None
    pages_with_filter = [
    ...
    if selected_page in pages_with_filter:
        # Fetch history filtered by the user's role and scope to populate the date filter
        history_df = db_manager.get_validation_history(
            user_role=user_role, 
            username=username, 
            managed_users=managed_users
