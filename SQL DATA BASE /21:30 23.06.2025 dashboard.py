        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                try:
                    ...
                    conn.commit()
                    logging.info("Successfully added 'disabled' column to users table.")
                except mysql.connector.Error as err:
...
def process_uploaded_file(uploaded_file, selected_date=None):
    try:
        ...
        try:
            # 1. Get all valid usernames from the database
            all_users_df = db_manager.get_all_users()
            known_users = set(all_users_df['username'].str.lower()) if not all_users_df.empty else set()
            
            # 2. Get all unique usernames from the 'Created user' column of the uploaded file
            if 'Created user' in df.columns:
                uploaded_users = set(df['Created user'].dropna().astype(str).str.lower())
                
                # 3. Find the users that are in the file but not in the database
                ghost_users = uploaded_users - known_users
                
                if ghost_users:
                    # The on-screen warning is now outside the notification logic, so it will always show
                    ghost_users_str = ", ".join(sorted(list(ghost_users)))
                    with summary_tab:
                         st.warning(f"👻 **Ghost Users Found:** The following users from the file do not exist in the system: `{ghost_users_str}`. A notification will be sent if a Super User is configured.")

                    # 4. Now, attempt to send the notification
                    super_user_accounts = db_manager.get_users_by_role('Super User')
                    
                    if super_user_accounts:
                        message = (f"During the processing of file '{uploaded_file.name}', the following usernames were found in the data "
                                   f"but do not exist in the system's user list: **{ghost_users_str}**. "
                                   f"Please consider adding them via the User Management page if they are valid users.")
                        
                        for user_account in super_user_accounts:
                            db_manager.create_notification(
                                username=user_account,
                                notif_type='Ghost User Detected',
                                message=message
                            )
                        logging.warning(f"Ghost users detected in {uploaded_file.name}: {ghost_users_str}. Notifying super users: {super_user_accounts}")
                    else:
                        # If no super users are found, log it but do not stop the process
                        logging.warning(f"Ghost users were detected in {uploaded_file.name}, but no 'Super User' accounts were found in the database to notify.")

        except Exception as e_ghost:
            logging.error(f"Error during ghost user detection for file {uploaded_file.name}: {e_ghost}", exc_info=True)
