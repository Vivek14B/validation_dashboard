...
def process_uploaded_file(uploaded_file, selected_date=None):
    try:
        ...
        # --- NEW: Role-Based Filtering of Uploaded Data ---
        ...
        if user_role == 'User':
            ...
            accessible_users = [username.lower()] + [u.lower() for u in managed_users]
            df_to_process = df_original[df_original['Created user'].str.lower().isin(accessible_users)].copy()
            filter_message = f"As a **Manager**, this file has been filtered to process records created by you and your team."
        ...
        missing_validator_cols_list = [col for col in validator_expected_columns if col not in df_to_process.columns]
        ...
        with summary_tab:
            ...
            display_metric("Departments", df_to_process['Department.Name'].nunique() if 'Department.Name' in df_to_process.columns else "N/A", container=col_info4)
        
        exceptions_df_from_validation = pd.DataFrame()
        department_statistics = {}

        with st.spinner(f"🔍 Validating {processed_record_count} records from {uploaded_file.name}..."):
        ...
        try:
            ...
            if 'Created user' in df_to_process.columns:
                ...
                if ghost_users:
                    ...
                    with summary_tab:
                         st.warning(f"👻 **Ghost Users Found:** The following users from the processed data do not exist in the system: `{ghost_users_str}`. A notification will be sent if a Super User is configured.")
                    ...
                    if super_user_accounts:
                        ...
                        for user_account in super_user_accounts:
                            db_manager.create_notification(username=user_account, notif_type='Ghost User Detected', message=message)
        ...
        with summary_tab:
            ...
            if exceptions_df_from_validation.empty:
                st.markdown(f'<div class="success-box"><strong>✅ Perfect!</strong> No validation issues found in the processed records from "{uploaded_file.name}"!</div>', unsafe_allow_html=True)
            else:
