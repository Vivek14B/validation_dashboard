...
def json_serializer_default(obj):
    # UNIVERSAL FIX: Use pandas' isna() to catch all types of "Not a Number" or "Not a Time"
    # This includes np.nan, None, pd.NaT, etc. This is the most robust way.
    if pd.isna(obj):
        return None

    # Handle specific data types for correct formatting
    ...
    if isinstance(obj, (np.floating, float)):
        # This will now only be called for actual numbers, not NaN
        return float(obj)
    ...
    # Fallback for other complex types to be converted to a string
    # It's better to convert them to strings than to let them cause an error.
    try:
        return str(obj)
    except Exception:
...
class DatabaseManager:
    def __init__(self, db_creds=st.secrets["mysql"]):
        """
        Initializes the DatabaseManager for MySQL.
        Credentials are now fetched from Streamlit's secrets.
        """
    ...
    def _get_connection(self):
        ...
        try:
            ...
            if err.errno == errorcode.ER_ACCESS_DENIED_ERROR:
                st.error("FATAL: MySQL access denied. Please check 'user' and 'password' in secrets.toml.")
            elif err.errno == errorcode.ER_BAD_DB_ERROR:
                st.error(f"FATAL: The database '{self.db_creds.get('database')}' does not exist.")
            else:
                st.error(f"FATAL: Could not connect to MySQL. Error: {err}")
            logging.critical(f"Database connection failed: {err}", exc_info=True)
            st.stop() # Stop the app if DB connection fails

    def init_database(self):
        """Initializes the tables in the MySQL database if they don't exist."""
        conn = self._get_connection()
        try:
            # For MySQL, it's good practice to specify the engine and charset
            table_options = "ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
            with conn.cursor() as cursor:
                # Use backticks (`) to quote table/column names for safety
                # Use INT PRIMARY KEY AUTO_INCREMENT for auto-incrementing keys
                # Use LONGBLOB for potentially large binary files (Excel reports)
                cursor.execute(f'''
                    CREATE TABLE IF NOT EXISTS `validation_runs` (
                        ...
                        `status` TEXT,
                        `file_size` BIGINT,
                        `excel_report_data` LONGBLOB
                    ) {table_options}''')
    
                # THE CHANGE IS IN THIS BLOCK
                cursor.execute(f'''
                    CREATE TABLE IF NOT EXISTS `exceptions` (
                        `id` INT PRIMARY KEY AUTO_INCREMENT,
                        `run_id` INT,
                        `department` TEXT, `sub_department` TEXT, `created_user` TEXT,
                        `modified_user` TEXT, `exception_reason` TEXT, `severity` INT,
                        `net_amount` FLOAT, `location` TEXT, `crop` TEXT, `activity` TEXT,
                        `function_name` TEXT, `vertical_name` TEXT, `region_name` TEXT,
                        `zone_name` TEXT, `business_unit` TEXT, `account2_code` TEXT,
                        `sub_ledger_code` TEXT, 
                        `original_row_data` TEXT,  # <-- THE FIX: Changed from JSON to TEXT
                ...
                cursor.execute(f'''
                    CREATE TABLE IF NOT EXISTS `department_summary` (
                        ...
                        `total_records` INT, `exception_records` INT, `exception_rate` FLOAT,
                        FOREIGN KEY (`run_id`) REFERENCES `validation_runs`(`id`) ON DELETE CASCADE
                    ) {table_options}''')
    
                # `user` is a reserved keyword in MySQL, so it must be quoted with backticks ``
                cursor.execute(f'''
                    CREATE TABLE IF NOT EXISTS `user_performance` (
                        `id` INT PRIMARY KEY AUTO_INCREMENT, `run_id` INT, `user` TEXT,
                        `total_records` INT, `exception_records` INT, `exception_rate` FLOAT,
                        FOREIGN KEY (`run_id`) REFERENCES `validation_runs`(`id`) ON DELETE CASCADE
                    ) {table_options}''')
                ...
                cursor.execute(f'''
                    CREATE TABLE IF NOT EXISTS `users` (
                        ...
                        `hashed_password` TEXT NOT NULL, `role` TEXT NOT NULL, `reports_to` TEXT
                    ) {table_options}''')
    
                cursor.execute(f'''
                    CREATE TABLE IF NOT EXISTS `correction_status` (
                        `id` INT PRIMARY KEY AUTO_INCREMENT, `run_id` INT NOT NULL,
                        `username` VARCHAR(255) NOT NULL,
                        `status` ENUM('Yes', 'No', 'Pending') NOT NULL,
                        `update_time` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                        FOREIGN KEY (`run_id`) REFERENCES `validation_runs`(`id`) ON DELETE CASCADE,
                        UNIQUE KEY `unique_run_user` (`run_id`, `username`)
                    ) {table_options}''')
            ...
            logging.error(f"Database initialization error (MySQL): {err}", exc_info=True)
            conn.rollback()
        finally:
            conn.close()
    ...
    def add_user(self, username, password, role, reports_to=None):
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute(
                    "INSERT INTO `users` (username, hashed_password, role, reports_to) VALUES (%s, %s, %s, %s)",
            ...
            logging.error(f"Error in add_user: {err}", exc_info=True)
    ...
    def get_all_users(self):
        conn = self._get_connection()
        try:
            # pd.read_sql_query works directly with the connection object
            df = pd.read_sql_query("SELECT id, username, role, reports_to FROM `users`", conn)
            return df
        except mysql.connector.Error as err:
            logging.error(f"Error in get_all_users: {err}", exc_info=True)
    ...
    def get_managed_users(self, manager_username):
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute("SELECT username FROM `users` WHERE reports_to = %s", (manager_username,))
                users = [row[0] for row in cursor.fetchall()]
        except mysql.connector.Error as err:
            logging.error(f"Error in get_managed_users: {err}", exc_info=True)
        finally:
            conn.close()
        return users
    ...
    def save_validation_run(self, filename, total_records, total_exceptions, file_size, upload_time=None):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                run_id = cursor.lastrowid # Use cursor.lastrowid for MySQL
                conn.commit()
        except mysql.connector.Error as err:
            logging.error(f"Error in save_validation_run: {err}", exc_info=True)
            conn.rollback()
            raise
        finally:
            conn.close()
    ...
    def save_excel_report(self, run_id, excel_data):
        ...
        try:
            with conn.cursor() as cursor:
                # mysql-connector handles bytes directly, no need for sqlite3.Binary
                cursor.execute('''UPDATE `validation_runs` SET excel_report_data = %s WHERE id = %s''',
                               (excel_data.getvalue(), run_id))
            conn.commit()
        except mysql.connector.Error as err:
            logging.error(f"Error saving Excel report for run_id {run_id}: {err}", exc_info=True)
            conn.rollback()
        finally:
            conn.close()
    ...
    def delete_run(self, run_id):
        ...
        try:
            with conn.cursor() as cursor:
                # Foreign key ON DELETE CASCADE handles deleting from child tables
                cursor.execute("DELETE FROM `validation_runs` WHERE id = %s", (run_id,))
            conn.commit()
            return True
        except mysql.connector.Error as err:
            logging.error(f"Error deleting run ID {run_id}: {err}", exc_info=True)
            conn.rollback()
            return False
        finally:
            conn.close()
        ...
        try:
            with conn.cursor() as cursor:
                ...
                for _, row_series in exceptions_df.iterrows():
                    ...
                    try:
                        # Use the original serializer, which is now safe because the column is TEXT
                        serialized_row_data = json.dumps(original_row_data_dict, default=json_serializer_default)
                    except Exception as json_e:
                        logging.error(f"Could not serialize row for run_id {run_id}. Error: {json_e}", exc_info=True)
                ...
                if data_to_insert:
                    cursor.executemany('''INSERT INTO `exceptions` (
                        ...
                        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)''', data_to_insert)
            conn.commit()
        except mysql.connector.Error as err:
            logging.error(f"Error in save_exceptions (MySQL): {err}", exc_info=True)
            conn.rollback()
            raise
        finally:
    ...
    def save_department_summary(self, run_id, department_statistics):
        """Saves department summary statistics for a given run."""
        conn = self._get_connection()
        try:
            with conn.cursor() as cursor:
                data_to_insert = []
                for dept, stats in department_statistics.items():
                    data_to_insert.append((
                        run_id, dept, stats['total_records'], 
                        stats['exception_records'], stats['exception_rate']
                    ))
                if data_to_insert:
                    cursor.executemany('''
            ...
            logging.error(f"Error saving department_summary for run {run_id}: {err}", exc_info=True)
            conn.rollback()
        finally:
            conn.close()
    ...
    def save_user_performance(self, run_id, df, exceptions_df):
        try:
            ...
            if total_records_user_df.empty:
                logging.warning(f"Run ID {run_id}: No valid users found. Cannot save performance.")
                return
                ...
                with conn.cursor() as cursor:
                    ...
                    if data_to_insert_perf:
                        cursor.executemany('''INSERT INTO `user_performance` (run_id, `user`, total_records, exception_records, exception_rate)
                                              VALUES (%s,%s,%s,%s,%s)''', data_to_insert_perf)
                        conn.commit()
                        logging.info(f"Successfully saved user performance for {len(data_to_insert_perf)} user(s) in run {run_id}.")
            except mysql.connector.Error as err:
                 logging.error(f"CRITICAL DB ERROR in save_user_performance for run ID {run_id}: {err}", exc_info=True)
                 conn.rollback()
            finally:
                conn.close()

        except Exception as e:
            logging.error(f"CRITICAL LOGIC ERROR in save_user_performance for run ID {run_id}: {e}", exc_info=True)
...
def process_uploaded_file(uploaded_file, selected_date=None):
    try:
        ...
        with st.spinner(f"📖 Reading file: {uploaded_file.name}..."):
            try:
                df = pd.read_excel(uploaded_file, engine='openpyxl', skiprows=5, skipfooter=1)
        ...
        # It now uses the proper manager method instead of a direct SQLite connection.
...
def show_upload_page():
    ...
    # ... (rest of the show_upload_page function is unchanged) ...
    st.markdown("---")
    ...
    with st.form("manual_data_entry_form"):
        ...
        # ... (rest of manual entry form is unchanged) ...
...
def show_user_performance_page(start_date, end_date):
    ...
    # --- 4. CORRECTED: Filter User Dropdown Based on Role AND Exceptions ---
    conn = db_manager._get_connection()
    unique_sorted_users = []
    try:
        placeholders = ','.join(['%s'] * len(run_ids_in_scope))
        # FIXED: This query now only selects users with exception_records > 0
        ...
        if user_role == 'Manager':
            if not managed_users: 
                ...
                conn.close()
                return
            user_placeholders = ','.join(['%s'] * len(managed_users))
    ...
    if not unique_sorted_users:
        st.info("No users with exceptions found for the selected scope.")
        return
    ...
    # --- 5. Display Logic (Complete and Corrected) ---
    if selected_user_for_perf == "All Users (Summary)":
        ...
        if not all_exc_df.empty:
            ...
            fig_mistakes_summary = px.bar(all_mistakes_df.head(15), x='Mistake Type', y='Count', title="Top 15 Mistake Types Across All Users", template="plotly_white")
            st.plotly_chart(fig_mistakes_summary, use_container_width=True)

        st.markdown("---")
        # FIXED: Email functionality is now fully implemented
        ...
        try:
            ...
            placeholders_sql = ','.join(['%s'] * len(run_ids_in_scope))
        ...
        if user_perf_df.empty:
            st.info(f"No performance data found for user '{selected_user_for_perf}' in the selected scope.")
            return
        ...
        if not user_exc_df.empty:
            ...
            fig_mistakes = px.pie(mistake_counts.head(10), names='Mistake Type', values='Count', title="Top Mistake Types Distribution", color_discrete_sequence=px.colors.qualitative.Pastel)
        ...
        if not user_perf_df.empty:
            ...
            fig_trend = px.line(user_perf_df, x='upload_time', y='exception_rate', markers=True, title="User Exception Rate Trend", labels={'upload_time': 'Date', 'exception_rate': 'Exception Rate (%)'})
        ...
        if not mistake_counts.empty:
            ...
            for index, row in top_mistakes_for_training.iterrows():
                ...
                if error_type in validator.training_map:
                    ...
                    st.markdown(f"- **For '{error_type}':** No specific training suggestion available.")
        
        st.markdown("---")
        # FIXED: Email functionality is now fully implemented
        with st.expander("📧 Email Performance Report"):
            with st.form("email_single_user_form"):
                to_input = st.text_input("To", value=selected_user_for_perf if '@' in selected_user_for_perf else "")
                ...
                <html style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
                <body style="background-color: #f4f4f4; margin: 0; padding: 20px;">
                    <h2 style="color: #333333;">Performance Summary for {selected_user_for_perf}</h2>
...
def show_report_archive_page():
    ...
    # --- 2. NEW: Multi-Select Widget for Runs ---
    ...
    # Use session state to manage the multiselect widget's state
    ...
    # Add a button to easily select all runs
    ...
    # --- 3. Fetch Data Based on Multi-Select ---
    with st.spinner("Loading report data for selected run(s)..."):
    ...
    # --- 4. User Filter (remains the same) ---
    st.markdown("---")
    ...
    elif user_role == 'Manager' and user_filter_selection == "All My Users":
        users_to_include = managed_users + [username]
        filtered_df = filtered_df[filtered_df['Created user'].isin(users_to_include)]
    elif user_filter_selection not in ["All My Users", "All Users (Unfiltered)"]:
        filtered_df = filtered_df[filtered_df['Created user'] == user_filter_selection]

    if filtered_df.empty:
        st.error("No transactions found for the selected filter combination.")
        return
                ...
                if submitted:
                    ...
                    if not to_recipients:
                        ...
                        with st.spinner("Attaching report and sending email..."):
                            # This calls the new helper function you added in the previous step
...
def show_correction_status_page(start_date, end_date):
    ...
    if todays_runs_df.empty:
        ...
        if selected_run_display:
            ...
            # UPDATED: Replaced direct sqlite3 connection with MySQL connection
            ...
            if not users_to_show:
                ...
                conn = db_manager._get_connection()
                try:
                    # Removed SQLite-specific 'COLLATE NOCASE'
                    query = "SELECT `user` FROM `user_performance` WHERE run_id = %s AND `user` = %s"
                    participant_df = pd.read_sql_query(query, conn, params=(selected_run_id, username))
                except mysql.connector.Error as e:
                    st.error(f"Database error checking user participation: {e}")
                finally:
                    if conn and conn.is_connected():
                        conn.close()
    ...
    # === Part 2: Analytics Section (This part uses the already-updated db_manager methods) ===
    if user_role in ["Manager", "Management"]:
        ...
        # This history_df is already defined above, so we can reuse it
        ...
        # This method was already updated to use MySQL
...
def main():
    ...
    # This function now uses the MySQL-compatible db_manager in the background
    ...
    if selected_page in pages_with_filter:
        # This now calls the MySQL-compatible get_validation_history method
        history_df = db_manager.get_validation_history(
            user_role=user_role, 
            username=username, 
            managed_users=managed_users
    ...
    # All these show_*_page functions have already been updated.
