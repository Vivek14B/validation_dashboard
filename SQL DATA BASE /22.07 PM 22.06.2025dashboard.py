...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                # UPDATED: users table with new optional email and mobile_number columns
                cursor.execute(f'''
                    CREATE TABLE IF NOT EXISTS `users` (
                        ...
                        `reports_to` TEXT,
                        `mapped_to_management` TEXT,
                        `can_upload` BOOLEAN DEFAULT NULL,
                        `disabled_pages` TEXT,
                        `email` VARCHAR(255) NULL,
                        `mobile_number` VARCHAR(20) NULL
                    ) {table_options}''')

                # (The rest of the table creation logic is unchanged)
                cursor.execute(f'''CREATE TABLE IF NOT EXISTS `role_permissions` (`role` VARCHAR(255) PRIMARY KEY, `can_upload` BOOLEAN DEFAULT TRUE, `disabled_pages` TEXT) {table_options}''')
                default_roles = ["User", "Manager", "Management", "Super User"]
                for role in default_roles:
                    cursor.execute("INSERT INTO `role_permissions` (role, can_upload, disabled_pages) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE role=role", (role, True, ''))
                ...
                # Secure Super User Creation from Environment Variables
                ...
                if cursor.fetchone()[0] == 0:
                    ...
                    if super_user and super_pass:
                        ...
                        cursor.execute("INSERT INTO `users` (username, hashed_password, role) VALUES (%s, %s, %s)",(super_user, hashed_password, 'Super User'))
                        conn.commit()
                        logging.info(f"Successfully created initial 'Super User' account for '{super_user}'.")
                    else:
                        st.warning("IMPORTANT: No users exist. Please set SUPER_USER_USERNAME and SUPER_USER_PASSWORD environment variables and restart to create the initial Super User account.")
        except mysql.connector.Error as err:
    ...
    # --- User and Permission Management Methods ---
    
    # UPDATED: To accept new optional email and mobile_number fields
    def add_user(self, username, password, role, reports_to=None, email=None, mobile_number=None):
        hashed_password = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
        conn = self._get_connection()
        try:
            with conn.cursor() as cursor:
                # UPDATED: The INSERT statement now includes the new columns
                sql = "INSERT INTO `users` (username, hashed_password, role, reports_to, email, mobile_number) VALUES (%s, %s, %s, %s, %s, %s)"
                val = (username, hashed_password, role, reports_to, email, mobile_number)
    ...
    # UPDATED: To select the new email and mobile_number columns
    def get_all_users(self):
        conn = self._get_connection()
        try:
            query = "SELECT id, username, role, reports_to, mapped_to_management, can_upload, disabled_pages, email, mobile_number FROM `users`"
...
def show_upload_page():
    ...
    # This value was calculated and stored in the session state by the main() function.
    # It checks the role permission and any user-specific override.
    ...
    with st.form("manual_data_entry_form"):
        ...
        # This form can be expanded with more fields as needed
        activity_manual_input = "" # Placeholder
        net_amount_manual_input = 0.0 # Placeholder
        sub_dept_manual_input = "" # Placeholder
        modified_user_manual_input = "" # Placeholder
        crop_manual_input = "" # Placeholder
        function_manual_input = "" # Placeholder
        vertical_manual_input = "" # Placeholder
        region_manual_input = "" # Placeholder
        zone_manual_input = "" # Placeholder
        business_unit_manual_input = "" # Placeholder
...
def show_user_performance_page(start_date, end_date):
    ...
    # --- 3. NEW HIERARCHICAL VIEW LOGIC ---
    
    # Simple case for a regular User
    if user_role == 'User':
        st.markdown(f"#### Your Performance Details")
        display_single_user_performance(username, run_ids_in_scope)
    ...
    # View for a Manager
        ...
        options = ["All My Team (Summary)"] + sorted(team_with_exceptions)
        selected_view = st.selectbox("Select View", options)

        if selected_view == "All My Team (Summary)":
            st.markdown(f"#### Performance Summary for **Your Team**")
            display_all_users_performance_summary(run_ids_in_scope, team_list)
        else:
            st.markdown(f"#### Performance Details for **{selected_view}**")
            display_single_user_performance(selected_view, run_ids_in_scope)

    # Global view for Management and Super User
        ...
        if view_type == "View All Users (Summary)":
            ...
            display_all_users_performance_summary(run_ids_in_scope, user_list=None)
        else:
            all_managers_df = db_manager.get_all_users()
            all_managers = all_managers_df[all_managers_df['role'] == 'Manager']['username'].tolist()
            if not all_managers:
                st.warning("No users with the 'Manager' role exist to filter by."); return
            
            selected_manager = st.selectbox("Select a Manager to view their team", options=[""] + sorted(all_managers))
            if selected_manager:
                manager_team = db_manager.get_managed_users(selected_manager)
                team_list = manager_team + [selected_manager]
                team_with_exceptions = get_filtered_users_with_exceptions(run_ids_in_scope, team_list)
                
                # NEW: Drill-down functionality
                if not team_with_exceptions:
                    st.info(f"No users in {selected_manager}'s team had exceptions in the selected scope.")
                    return
                
                drill_down_options = [f"Summary for {selected_manager}'s Team"] + sorted(team_with_exceptions)
                selected_drill_down = st.selectbox("Select View for this Team", options=drill_down_options)
                
                if selected_drill_down == f"Summary for {selected_manager}'s Team":
                    st.markdown(f"#### Performance Summary for **{selected_manager}'s Team**")
                    display_all_users_performance_summary(run_ids_in_scope, team_list)
                else:
                    st.markdown(f"#### Performance Details for **{selected_drill_down}**")
                    display_single_user_performance(selected_drill_down, run_ids_in_scope)
def get_filtered_users_with_exceptions(run_ids, user_list=None):
    ...
    within a given set of runs and optionally filtered by a user list.
    """
    ...
    try:
        ...
        if conn and conn.is_connected():
            conn.close()
    return unique_sorted_users


# We create new helper functions to avoid repeating code
def display_single_user_performance(user, run_ids):
    """Displays the detailed performance page for a single user."""
    conn = db_manager._get_connection()
    try:
        params = [user] + run_ids
    ...
    with st.expander("📧 Email Performance Report"):
        with st.form(f"email_single_user_form_{user.replace('.', '_')}"):
            ...
            <body style="background-color: #f4f4f4; margin: 0; padding: 20px;">
                <h2 style="color: #333333;">Performance Summary for {user}</h2>
...
def display_all_users_performance_summary(run_ids, user_list=None):
    """Displays the summary performance for a list of users."""
    conn = db_manager._get_connection()
    try:
        placeholders = ','.join(['%s'] * len(run_ids))
...
def show_access_control_page():
    ...
    # Get all page names for the multiselect widgets
    # This list must be maintained if you add/remove/rename pages
    all_pages = [
        "🏠 Upload & Validate", "📊 Dashboard Analytics", "📈 Trends & History",
        "👤📊 User Performance", "🗂️ Report Archive", "📝 Correction Status",
        "📋 Exception Details", "📍 Location Expenses", "🧾 Ledger/Sub-Ledger Summary",
        "👤🧾 User-wise Ledger Exceptions", "👤📍 User & Location Analysis", "🗑️ Data Management",
        "⚙️ Settings", "🛠️ User Management", "🔑 Access Control"
    ]
    
    # Fetch all current permissions from the database once
    permissions = db_manager.get_all_permissions()
    all_users_df = db_manager.get_all_users()

    if not permissions['roles']:
        st.error("Role permissions could not be loaded from the database.")
        return
    ...
    with tab1:
        ...
        # We don't allow editing the Super User role's permissions
        ...
        for role in roles_to_manage:
            with st.expander(f"Permissions for **{role}** Role"):
                ...
                # Filter out the upload page from being disabled, as it's controlled by the toggle
                pages_for_role_config = [p for p in all_pages if p != "🏠 Upload & Validate"]
                
                new_disabled_pages_role = st.multiselect(
                    "Select Dashboards to DISABLE for this Role:",
                    options=pages_for_role_config,
    ...
    with tab2:
        ...
        if not users_to_manage:
            ...
            selected_user = st.selectbox("Select a user to manage their specific permissions", options=[""] + users_to_manage)
            
            if selected_user:
                current_user_perms = permissions['users'].get(selected_user, {})
                
                # --- Upload Permission Override ---
                can_upload_user = current_user_perms.get('can_upload', None) # None means inherit
                
                # Map database value (True, False, None) to radio button index (1, 2, 0)
                upload_override_options = ["Inherit from Role", "Always Allow", "Always Deny"]
                current_upload_index = 0 # Default to "Inherit"
                if can_upload_user is True:
                    current_upload_index = 1
                elif can_upload_user is False:
                ...
                pages_for_user_config = [p for p in all_pages if p != "🏠 Upload & Validate"]
                
                new_disabled_pages_user = st.multiselect(
                    f"Select Dashboards to specifically DISABLE for **{selected_user}**:",
                    options=pages_for_user_config,
                    default=disabled_pages_user_list,
                    key=f"pages_user_{selected_user}"
                )
                st.caption("Note: This list overrides the role's disabled pages. An empty list means the user inherits the role's page visibility.")
...
def show_user_management_page():
    ...
    st.info("From this page, you can view all users, create new accounts, change roles, and manage mappings.")
    ...
    # Use 4 tabs for better organization
    tab1, tab2, tab3, tab4 = st.tabs(["👥 View All Users", "➕ Create New User", "✏️ Edit User & Map to Manager", "🗺️ Map Manager to Management"])

    with tab1:
        st.markdown("#### Current Users in the System")
        if not all_users_df.empty:
            # UPDATED: Added email and mobile_number to the display
            st.dataframe(
                all_users_df,
                use_container_width=True,
                hide_index=True,
                column_order=("id", "username", "role", "email", "mobile_number", "reports_to", "mapped_to_management", "can_upload", "disabled_pages"),
                column_config={
                    "id": "ID", "username": "Username", "role": "Role",
                    "email": "Email", "mobile_number": "Mobile",
                    "reports_to": "Reports To (Manager)", "mapped_to_management": "Mapped To (Mgmt)",
                    "can_upload": "Can Upload?", "disabled_pages": "Disabled Dashboards"
                }
            )
        else:
    ...
    with tab2:
        ...
        with st.form("create_user_form"):
            ...
            st.markdown("---")
            # NEW: Optional fields for email and mobile
            st.markdown("##### Optional Information")
            new_email = st.text_input("Email (for notifications)")
            new_mobile = st.text_input("Mobile Number")
            
            st.markdown("---")
            # NEW: Checkbox to send credentials
            send_credentials_checkbox = st.checkbox("Send login details to user via email", value=False)
            st.caption("Note: An email address must be provided above for this feature to work.")
            ...
            if submitted:
                if not new_username or not new_password:
                    ...
                    # UPDATED: Pass new fields to the add_user function
                    ...
                    if result is True:
                        ...
                        # NEW: Logic to send the email if the box is checked
                        if send_credentials_checkbox:
                            if new_email:
                                ...
                                st.warning("Could not send welcome email: No email address was provided.")
    ...
    with tab3:
        st.markdown("#### Edit User Role and Manager Assignment")
        if not all_users_list:
            st.warning("No users exist to be edited.")
        else:
            user_to_edit = st.selectbox("Select User to Edit", [""] + all_users_list, index=0, key="user_edit_select", help="Edit a user's role or assign a direct manager to them.")

            if user_to_edit:
                st.markdown(f"---")
                st.markdown(f"**Editing:** `{user_to_edit}`")
                
                current_user_data = all_users_df[all_users_df['username'] == user_to_edit].iloc[0]
                ...
                if new_role_edit == 'User':
                    current_manager = current_user_data['reports_to']
                    manager_options = ["None"] + manager_list
                    try:
                        current_manager_index = manager_options.index(current_manager) if pd.notna(current_manager) and current_manager in manager_options else 0
                    except ValueError:
                        current_manager_index = 0

                    new_manager_map = st.selectbox(f"Map '{user_to_edit}' to Manager", manager_options, index=current_manager_index)
                    ...
                    st.info(f"Assigning a direct manager is only applicable for the 'User' role.")
                
                st.markdown("---")
                
                with st.expander("🔑 Change Password"):
                    with st.form(f"change_password_form_{user_to_edit.replace('.', '_')}"):
    ...
    with tab4:
        st.markdown("#### Map a Manager to a Management User")
        st.caption("This is an optional mapping for reporting and organizational structure.")

        if not manager_list:
            st.warning("No users with the 'Manager' role exist to be mapped.")
        elif not management_list:
            st.warning("No users with the 'Management' role exist to be mapped to.")
        else:
            manager_to_map = st.selectbox("Select a Manager to map", options=[""] + manager_list, key="manager_map_select")
            
            if manager_to_map:
                current_management_mapping = all_users_df[all_users_df['username'] == manager_to_map]['mapped_to_management'].iloc[0]
                management_options = ["None"] + management_list
                
                try:
                    current_map_index = management_options.index(current_management_mapping) if pd.notna(current_management_mapping) and current_management_mapping in management_options else 0
                except ValueError:
                    current_map_index = 0

                new_management_map = st.selectbox(f"Map '{manager_to_map}' to Management User", options=management_options, index=current_map_index, key="management_map_select")
                
                if st.button("Update Manager-to-Management Mapping"):
                    db_manager.update_manager_to_management_mapping(manager_to_map, new_management_map)
                    st.success(f"Successfully mapped '{manager_to_map}' to '{new_management_map}'.")
                    st.rerun()
...
def main():
    ...
    # --- UPDATED HIERARCHICAL PERMISSION LOGIC ---
    user_permissions = None

    # NEW: Add a special case to grant Super User all permissions automatically
    if user_role == "Super User":
        user_permissions = {
            "can_upload": True,
            "disabled_pages": [] # An empty list means no pages are disabled
        }
    else:
        # For all other roles, fetch their specific permissions from the database
        user_permissions = db_manager.get_user_permissions(username)

    if user_permissions is None:
        st.error("Could not load your user permissions. Please contact an administrator.")
        st.stop()
    
    # Store the final permissions in the session state for other pages to use
    st.session_state['can_upload'] = user_permissions['can_upload']
    user_disabled_pages = user_permissions['disabled_pages']
    # --- END OF UPDATED PERMISSION LOGIC ---
    ...
    page_access = {
        "Super User": [
            ...
            "👤📊 User Performance", "🗂️ Report Archive", "📝 Correction Status",
            "📋 Exception Details", "📍 Location Expenses", "🧾 Ledger/Sub-Ledger Summary",
            "👤🧾 User-wise Ledger Exceptions", "👤📍 User & Location Analysis", 
            "🔑 Access Control", "🛠️ User Management", "🗑️ Data Management", "⚙️ Settings"
        ],
        "Management": [
            "🏠 Upload & Validate", "📊 Dashboard Analytics", "📈 Trends & History",
            "👤📊 User Performance", "🗂️ Report Archive", "📝 Correction Status",
        ...
        "User": [
            "🏠 Upload & Validate", "👤📊 User Performance", "🗂️ Report Archive", "📝 Correction Status"
        ]
    }
    
    base_pages_for_role = page_access.get(user_role, [])
    
    # Filter the base list against the user's specific disabled pages
