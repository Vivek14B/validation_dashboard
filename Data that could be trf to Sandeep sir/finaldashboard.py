...
class DatabaseManager:
    ...
    # This entire block of code MUST be indented under the 'def' line
        conn = None
        try:
            ...
            if cursor.fetchone()[0] == 0:
                ...
                cursor.execute(
                    ...
                    (default_username, hashed_password, 'Management')
                )
                conn.commit()
                logging.info(f"Created default '{default_username}' user with 'Management' role.")
    ...
    def add_user(self, username, password, role, reports_to=None):
        hashed_password = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
        conn = sqlite3.connect(self.db_path, timeout=DB_TIMEOUT)
        try:
            conn.execute(
                "INSERT INTO users (username, hashed_password, role, reports_to) VALUES (?, ?, ?, ?)",
                (username, hashed_password, role, reports_to)
            )
            conn.commit()
            return True
    ...
    def get_all_users(self):
        conn = sqlite3.connect(self.db_path, timeout=DB_TIMEOUT)
        df = pd.read_sql_query("SELECT id, username, role, reports_to FROM users", conn)
        conn.close()
        return df
    ...
    def get_managed_users(self, manager_username):
        conn = sqlite3.connect(self.db_path, timeout=DB_TIMEOUT)
        cursor = conn.cursor()
        cursor.execute("SELECT username FROM users WHERE reports_to = ?", (manager_username,))
    ...
    def update_user_mapping(self, username, manager_username):
        ...
        conn = sqlite3.connect(self.db_path, timeout=DB_TIMEOUT)
        conn.execute("UPDATE users SET reports_to = ? WHERE username = ?", (manager, username))
        conn.commit()
        conn.close()
    ...
    # --- Original Data Saving and Retrieval Methods ---
        ...
        try:
            ...
            for _, row_series in exceptions_df.iterrows():
                ...
                data_to_insert.append(
                    (run_id,
                     row_series.get('Department.Name', ''), row_series.get('Sub Department.Name', ''),
                     ...
                     row_series.get('Sub Ledger.Code', ''), serialized_row_data
                    )
                )
    ...
    def save_user_performance(self, run_id, df, exceptions_df):
        """
        Calculates and saves performance statistics for each user in a given run.
        This robust version ensures all users from the source file are processed.
        """
        try:
            ...
            # More robustly clean the 'Created user' column
            df['Created user'] = df['Created user'].fillna('').astype(str).str.strip()

            # 1. Establish the source of truth: all users and their total record counts from the original file.
            ...
            if total_records_user_df.empty:
                logging.warning(f"Run ID {run_id}: No valid users found in 'Created user' column. Cannot save performance.")
                return

            # 2. Calculate exception records.
            if exceptions_df.empty or 'Created user' not in exceptions_df.columns:
                # If there are no exceptions at all, create a DataFrame with 0 exceptions for every user.
                exception_records_df = pd.DataFrame({
                    'Created user': total_records_user_df['Created user'],
                    'exception_records': 0
                })
            else:
                ...
                # Count exceptions only for users who have them.
                ...
                # Merge the exception counts with the full list of users to ensure users with zero exceptions are included.
                exception_records_df = pd.merge(
                    total_records_user_df[['Created user']], 
                    exception_counts, 
                    on='Created user', 
                    how='left'
                ).fillna(0)

            # 3. Combine the total records and exception records. This merge is now safe.
            user_stats = pd.merge(total_records_user_df, exception_records_df, on='Created user', how='left')

            # 4. Add logging to see the intermediate data during execution.
            logging.info(f"--- Debugging User Performance for Run ID: {run_id} ---")
            logging.info(f"Total Records DF Head:\n{total_records_user_df.head().to_string()}")
            logging.info(f"Exception Records DF Head:\n{exception_records_df.head().to_string()}")
            logging.info(f"Final Merged User Stats DF Head:\n{user_stats.head().to_string()}")

            if user_stats.empty:
                logging.error(f"Run ID {run_id}: Final user_stats DataFrame became empty before saving. Aborting save.")
                return

            # 5. Calculate the final rate and save to the database.
            ...
            if data_to_insert_perf:
                ...
                conn.commit()
                logging.info(f"Successfully saved user performance for {len(data_to_insert_perf)} user(s) in run {run_id}.")
            conn.close()

        except Exception as e:
            logging.error(f"CRITICAL ERROR in save_user_performance for run ID {run_id}: {e}", exc_info=True)
    ...
    def get_archived_report(self, run_id, user_role=None, username=None, managed_users=None):
        ...
        try:
            ...
            if user_role == 'Management':
                ...
                cursor.execute("SELECT 1 FROM user_performance WHERE run_id = ? AND user = ?", (run_id, username))
                if cursor.fetchone(): has_access = True
            elif user_role == 'Manager' and managed_users:
                placeholders = ','.join('?' for _ in managed_users)
                query = f"SELECT 1 FROM user_performance WHERE run_id = ? AND user IN ({placeholders})"
    ...
    def get_exceptions_by_run(self, run_id):
        ...
        try:
            conn = sqlite3.connect(self.db_path, timeout=DB_TIMEOUT)
            query = "SELECT id, run_id, exception_reason, severity, original_row_data FROM exceptions WHERE run_id = ?"
            ...
            for _, db_row in raw_exceptions_df.iterrows():
                try:
                    # Start with the original data from the JSON blob
                    record = json.loads(db_row['original_row_data']) if pd.notna(db_row['original_row_data']) else {}
                except json.JSONDecodeError:
                    record = {}

                # Add the key database fields. This will overwrite any identically named
                # fields from the JSON, which is what we want for these specific columns.
                record['id'] = db_row['id']
            ...
            # Return the full dataframe without trying to reorder it here.
            ...
            if conn: conn.close()
    # Add this entire block of code inside your DatabaseManager class

    # Place this entire block of code inside the DatabaseManager class

    def add_or_update_correction_status(self, run_id, username, status):
        """Inserts or updates a user's correction status for a specific run."""
        conn = None
        try:
            conn = sqlite3.connect(self.db_path, timeout=DB_TIMEOUT)
            cursor = conn.cursor()
            ...
            logging.error(f"Error updating correction status for run {run_id}, user {username}: {e}", exc_info=True)
            if conn:
                conn.rollback()
            return False
        finally:
    ...
    def get_correction_status_for_run(self, run_id):
        ...
        try:
            conn = sqlite3.connect(self.db_path, timeout=DB_TIMEOUT)
            query = "SELECT username, status FROM correction_status WHERE run_id = ?"
            df = pd.read_sql_query(query, conn, params=(run_id,))
            # Return as a dictionary for easy lookup: {'user1': 'Yes', 'user2': 'No'}
            return pd.Series(df.status.values, index=df.username).to_dict()
        except sqlite3.Error as e:
            logging.error(f"Error fetching correction status for run {run_id}: {e}", exc_info=True)
            return {}
        finally:
            if conn:
                conn.close()
    ...
    def get_correction_summary(self, run_ids, accessible_users=None):
        ...
        correctly defaulting to 'Pending' for today's runs and 'No' for past runs.
        """
        if not run_ids:
            return pd.DataFrame()
        ...
        try:
            ...
            # 1. Get all users who participated in the selected runs and are accessible
            perf_query = f"SELECT DISTINCT run_id, user FROM user_performance WHERE run_id IN ({placeholders})"
            params = list(run_ids)
            if accessible_users:
                user_placeholders = ','.join('?' for user in accessible_users)
                perf_query += f" AND user IN ({user_placeholders}) COLLATE NOCASE"
            ...
            # 2. Get the statuses that have been explicitly set
            ...
            # 3. Merge participant data with status data
            summary_df = pd.merge(
                all_participants_df, 
                status_df, 
                how='left', 
                left_on=['run_id', 'user'], 
                right_on=['run_id', 'username']
            )

            # 4. Determine which runs are from today
            ...
            # Added format='mixed' to handle cases where upload_time is just a date (from the override)
            # or a full timestamp (from a normal upload).
            ...
            # 5. Apply the default status logic
            def determine_status(row):
                if pd.notna(row['status']):
                    return row['status']  # A status was explicitly set
                if row['run_id'] in todays_run_ids:
                    return 'Pending'  # No status set for a run from today
                else:
                    return 'No'  # No status set for a run from a past day
...
def check_password():
    ...
    # The .get() method safely checks for the key without causing an error if it doesn't exist.
    ...
    if st.button("Log in", key="login_button"):
        ...
        if user_data:
            ...
            if bcrypt.checkpw(password_input.encode('utf-8'), stored_hashed_password):
                # --- On successful login, set the session state variables ---
                st.session_state["authentication_status"] = True
                st.session_state["username_actual"] = user_data[0]
                st.session_state["role"] = user_data[2]
                st.session_state["managed_users"] = db_manager.get_managed_users(user_data[0]) if user_data[2] == "Manager" else []
...
def show_user_performance_page(start_date, end_date):
    ...
    # --- 2. Fetch History Filtered by User's Permissions ---
    history_df = db_manager.get_validation_history(
        user_role=user_role, 
        username=username, 
    ...
    # Apply date filter to the permission-filtered data
    if start_date and end_date:
    ...
    # --- 3. Run Selector (Uses the filtered history_df) ---
    run_options = {f"Run {row['id']}: {row['filename']} ({pd.to_datetime(row['upload_time']).strftime('%Y-%m-%d %H:%M')})": row['id'] for _, row in history_df.iterrows()}
    run_options_with_all = {"All Runs (Summary)": "all", **run_options}
    ...
    selected_run_id = run_options_with_all[selected_run_display]
    run_ids_in_scope = history_df['id'].tolist() if selected_run_id == "all" else [selected_run_id]

    if not run_ids_in_scope:
        st.info("No runs match the selected criteria.")
        return

    # --- 4. Filter User Dropdown Based on Role ---
    conn = None
    try:
        conn = sqlite3.connect(db_manager.db_path, timeout=DB_TIMEOUT)
        placeholders = ','.join('?' for _ in run_ids_in_scope)
        base_query = f"SELECT DISTINCT user FROM user_performance WHERE run_id IN ({placeholders}) AND user IS NOT NULL AND user != ''"
        
        query_params = list(run_ids_in_scope)
        if user_role == 'Manager':
            if not managed_users: 
                st.warning("You are not assigned to any users to view.")
                conn.close()
                return
            user_placeholders = ','.join('?' for _ in managed_users)
            base_query += f" AND user IN ({user_placeholders})"
            query_params.extend(managed_users)
    ...
    # User Selector Dropdown (now correctly filtered)
    ...
    # --- 5. Display Logic (Your original code fully integrated) ---

    # --- "All Users" (Summary) View ---
    if selected_user_for_perf == "All Users (Summary)":
        st.markdown(f"#### Performance Summary for **All Accessible Users** (Scope: {selected_run_display})")
        
        conn = None
        try:
            conn = sqlite3.connect(db_manager.db_path, timeout=DB_TIMEOUT)
            perf_query = f"SELECT * FROM user_performance WHERE run_id IN ({placeholders})"
            all_perf_df = pd.read_sql_query(perf_query, conn, params=tuple(run_ids_in_scope))
            exc_query = f"SELECT created_user, exception_reason FROM exceptions WHERE run_id IN ({placeholders})"
            all_exc_df = pd.read_sql_query(exc_query, conn, params=tuple(run_ids_in_scope))
        except sqlite3.Error as e:
        ...
        # Final filtering for managers to be certain only their users are shown in summary
        if user_role == 'Manager':
            all_perf_df = all_perf_df[all_perf_df['user'].isin(managed_users)]
            all_exc_df = all_exc_df[all_exc_df['created_user'].isin(managed_users)]
...
def show_report_archive_page():
    ...
    st.info("Select a single validation run for a specific report, or choose 'All Runs' to download a consolidated report containing data from all accessible runs.")
    ...
    # --- 2. Create the Run Selector with the "All Runs" Option ---
    ...
    # Add the "All Runs" option to the beginning of the list
    all_options = ["All Runs (Consolidated Report)"] + list(run_options_dict.keys())
    
    selected_run_display = st.selectbox(
        "STEP 1: Select a Report Scope",
        options=all_options,
        index=None,
        placeholder="Choose a report to process..."
    )

    if selected_run_display:
        
        # --- 3. Fetch Data Based on Selection (Single Run vs. All Runs) ---
        unfiltered_df = pd.DataFrame()
        spinner_message = "Loading report data..."
        
        if selected_run_display == "All Runs (Consolidated Report)":
            spinner_message = "Loading data for all accessible runs..."
        ...
        with st.spinner(spinner_message):
            if selected_run_display == "All Runs (Consolidated Report)":
                ...
                all_exceptions_list = []
                for run_id in run_ids_to_load:
                    df_single_run = db_manager.get_exceptions_by_run(run_id)
                    if not df_single_run.empty:
                        all_exceptions_list.append(df_single_run)
                
                if all_exceptions_list:
                    unfiltered_df = pd.concat(all_exceptions_list, ignore_index=True)
            else: # A single run was selected
                selected_run_id = run_options_dict[selected_run_display]
                unfiltered_df = db_manager.get_exceptions_by_run(selected_run_id)

        if unfiltered_df.empty:
            st.warning("The selected scope contains no exception records to report.")
            return

        st.markdown("---")
        st.markdown("#### STEP 2: Choose How to Filter the Report")

        users_in_this_report = sorted(unfiltered_df['Created user'].dropna().unique())
        user_filter_selection = None

        # This filtering UI works for both single and all-run data
        if user_role == 'User':
            st.success(f"✔️ Your report will be automatically filtered for your user: **{username}**")
            user_filter_selection = username
        
        elif user_role == 'Manager':
            relevant_users_for_manager = [u for u in users_in_this_report if u in managed_users or u == username]
            options = ["All My Users"] + relevant_users_for_manager
            user_filter_selection = st.selectbox("Download report for:", options)

        elif user_role == 'Management':
            options = ["All Users (Unfiltered)"] + users_in_this_report
            user_filter_selection = st.selectbox("Download report for:", options)

        st.markdown("---")
        st.markdown("#### STEP 3: Generate and Download")

        # --- 4. The Download Logic (Works for both single and all-run data) ---
        download_label = f"📥 Prepare & Download Report"
        if st.button(download_label, type="primary"):
            
            with st.spinner("Generating your customized Excel report..."):
                filtered_df = unfiltered_df.copy()
                if user_role == 'User':
                    filtered_df = filtered_df[filtered_df['Created user'] == username]
                
                elif user_role == 'Manager':
                    if user_filter_selection != "All My Users":
                        filtered_df = filtered_df[filtered_df['Created user'] == user_filter_selection]
                    else:
                        users_to_include = managed_users + [username]
                        filtered_df = filtered_df[filtered_df['Created user'].isin(users_to_include)]

                elif user_role == 'Management':
                    if user_filter_selection != "All Users (Unfiltered)":
                        filtered_df = filtered_df[filtered_df['Created user'] == user_filter_selection]

                if filtered_df.empty:
                    st.error("No transactions found for the selected filter.")
                    st.stop()

                special_cols = ['id', 'run_id', 'Exception Reasons', 'Severity', 'original_row_data']
                original_cols = [col for col in unfiltered_df.columns if col not in special_cols]
                ...
                if final_report_data:
                    scope_name = "All_Runs" if selected_run_display == "All Runs (Consolidated Report)" else f"Run_{run_options_dict[selected_run_display]}"
                    report_filename = f"Filtered_Report_{scope_name}_{user_filter_selection.replace(' ', '_')}.xlsx"
                    ...
                    st.error("There was an error generating the Excel file.")

            # Show the download button outside the spinner block
            if 'download_info' in st.session_state and st.session_state.download_info:
                st.download_button(
                    ...
                    file_name=st.session_state.download_info['name'],
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    key="final_download_button"
                )
                # Clear the state after showing the button to prevent re-triggering
...
# In dashboard.py, add this new function with the other page functions
...
def show_correction_status_page(start_date, end_date):
    ...
    # === Part 1: Status Update Section (for today's runs) ===
    st.markdown("#### Update Status for Today's Runs")
    st.caption("You can only update the status for validation runs that occurred today.")
    today = datetime.now().date()
    
    history_df = db_manager.get_validation_history(user_role, username, managed_users)
    todays_runs_df = history_df[pd.to_datetime(history_df['upload_time']).dt.date == today].copy()

    if todays_runs_df.empty:
        st.info("No validation runs from today are available for you to update.")
    else:
        todays_runs_df['display_name'] = "Run " + todays_runs_df['id'].astype(str) + ": " + todays_runs_df['filename']
        selected_run_display = st.selectbox(
            "Select a run from today to update status:",
            options=todays_runs_df['display_name'].tolist(),
            index=None,
            placeholder="Choose a run..."
        )

        if selected_run_display:
            selected_run_id = int(todays_runs_df[todays_runs_df['display_name'] == selected_run_display]['id'].iloc[0])

            # --- MODIFIED LOGIC: ONLY FETCH USERS WITH EXCEPTIONS ---
            # This query now includes "AND exception_records > 0" to get only users with errors.
            base_query = "SELECT DISTINCT user FROM user_performance WHERE run_id = ? AND exception_records > 0 AND user IS NOT NULL AND user != ''"
            ...
            # --- New logic to show a friendly message if there are no exceptions for the user ---
            if not users_to_show:
                # Check if the user participated in the run at all
                conn = sqlite3.connect(db_manager.db_path, timeout=DB_TIMEOUT)
                participant_df = pd.read_sql_query("SELECT user FROM user_performance WHERE run_id = ? AND user = ? COLLATE NOCASE", conn, params=(selected_run_id, username))
                conn.close()
                
                if not participant_df.empty:
                    # This message shows if the user was in the run but had 0 exceptions
                    st.success("✅ You have no exceptions to correct for this run.")
                else:
                    # This message shows if the user was not involved in this run at all
                    st.warning("No users with exceptions associated with you were found in this validation run.")
            else:
    ...
    # === Part 2: Analytics Section (Identical to before) ===
    if user_role in ["Manager", "Management"]:
        # ... (The analytics part of the function remains unchanged) ...
        st.markdown("---")
        ...
        user_scope = (managed_users + [username]) if user_role == 'Manager' else None
...
def show_user_management_page():
    ...
    st.info("From this page, you can view all users, create new accounts, change roles, update passwords, and map users to their managers.")

    db_manager = get_database_manager()

    # Fetch current user data to populate UI elements
    all_users_df = db_manager.get_all_users()
    all_users_list = all_users_df['username'].tolist()
    manager_list = all_users_df[all_users_df['role'] == 'Manager']['username'].tolist()

    tab1, tab2, tab3 = st.tabs(["👥 View All Users", "➕ Create New User", "✏️ Edit User & Map to Manager"])
    ...
    with tab1:
        ...
        st.dataframe(
            ...
            use_container_width=True,
            hide_index=True,
            column_order=("id", "username", "role", "reports_to"),
            column_config={
                "id": "ID",
                "username": "Username",
                "role": "Role",
    ...
    with tab2:
        ...
        with st.form("create_user_form"):
            ...
            new_role = st.selectbox("Role", ["User", "Manager", "Management"])
            ...
            if submitted:
                if not new_username or not new_password:
                    ...
                    result = db_manager.add_user(new_username, new_password, new_role)
                    if result is True:
                        st.success(f"User '{new_username}' created successfully!")
                        st.rerun() # Rerun to refresh the user list in other tabs
                    else:
                        st.error(result)

    with tab3:
        st.markdown("#### Edit an Existing User")
        # Add a placeholder for the selectbox to prevent auto-selecting the first user
        user_to_edit = st.selectbox("Select User to Edit", [""] + all_users_list, index=0, key="user_edit_select")

        if user_to_edit:
            st.markdown(f"---")
            st.markdown(f"**Editing:** `{user_to_edit}`")
            
            # --- Change Role ---
            current_role = all_users_df[all_users_df['username'] == user_to_edit]['role'].iloc[0]
            role_options = ["User", "Manager", "Management"]
            try:
                current_role_index = role_options.index(current_role)
            except ValueError:
                current_role_index = 0
            
            new_role_edit = st.selectbox("Change Role", role_options, index=current_role_index)
            if st.button("Update Role"):
                db_manager.update_user_role(user_to_edit, new_role_edit)
                st.success(f"Role for '{user_to_edit}' updated to '{new_role_edit}'.")
                st.rerun()

            st.markdown("---")
            
            # --- Map to Manager (only if the user is not a manager themselves) ---
            if new_role_edit == 'User':
                current_manager = all_users_df[all_users_df['username'] == user_to_edit]['reports_to'].iloc[0]
                manager_options = ["None"] + manager_list
                try:
                    # Handle case where current_manager might not be in the list anymore
                ...
                new_manager_map = st.selectbox(
                    f"Map '{user_to_edit}' to Manager", 
                    manager_options, 
                    index=current_manager_index
                )
                if st.button("Update Manager Mapping"):
                    db_manager.update_user_mapping(user_to_edit, new_manager_map)
                    st.success(f"'{user_to_edit}' now reports to '{new_manager_map}'.")
                    st.rerun()
            else:
                st.info(f"Mapping is not applicable for the '{new_role_edit}' role.")
            ...
            with st.expander("🔑 Change Password"):
                with st.form("change_password_form"):
                    password_to_change = st.text_input("New Password", type="password", key="new_pass_edit")
                    if st.form_submit_button("Set New Password"):
                        if password_to_change:
                            db_manager.update_user_password(user_to_edit, password_to_change)
                            st.success(f"Password for '{user_to_edit}' has been changed.")
                        else:
...
def main():
    ...
    # If the user is not authenticated, it will show the login form and stop the script here.
    if not check_password():
        st.stop()

    # --- 2. GET USER CONTEXT AND SETUP SIDEBAR (This code runs only after a successful login) ---
    user_role = st.session_state.get("role")
    username = st.session_state.get("username_actual")
    managed_users = st.session_state.get("managed_users", [])
    ...
    if st.sidebar.button("Logout"):
        # On logout, clear all session state keys to ensure a clean slate
    ...
    # --- 3. ROLE-BASED ACCESS CONTROL (RBAC) FOR NAVIGATION ---
    # This dictionary defines which pages each role is allowed to see.
    page_access = {
        "User": [
            "🏠 Upload & Validate",
            "🗂️ Report Archive",
    ...
    # Get the list of pages permitted for the current user's role
    page_navigation_options = page_access.get(user_role, [])
    ...
    # --- 4. CENTRALIZED DATE FILTER LOGIC ---
    start_date, end_date = None, None
    pages_with_filter = [
        ...
        "👤📍 User & Location Analysis", "👤📊 User Performance", "📝 Correction Status"
    ]
    ...
    # --- 5. PAGE ROUTING ---
    # This block calls the correct function to display the selected page.
    if selected_page == "🏠 Upload & Validate":
        show_upload_page()
    elif selected_page == "📊 Dashboard Analytics":
        show_analytics_page(start_date, end_date)
    elif selected_page == "📈 Trends & History":
        show_trends_page(start_date, end_date)
    elif selected_page == "👤📊 User Performance":
        show_user_performance_page(start_date, end_date)
    elif selected_page == "🗂️ Report Archive":
        show_report_archive_page()
    elif selected_page == "📝 Correction Status":
        show_correction_status_page(start_date, end_date)
    elif selected_page == "📋 Exception Details":
        show_exception_details_page(start_date, end_date)
    elif selected_page == "📍 Location Expenses":
        show_location_expenses_page(start_date, end_date)
    elif selected_page == "🧾 Ledger/Sub-Ledger Summary":
        show_ledger_summary_page(start_date, end_date)
    elif selected_page == "👤🧾 User-wise Ledger Exceptions":
        show_user_ledger_exceptions_page(start_date, end_date)
    elif selected_page == "👤📍 User & Location Analysis":
        show_user_location_page(start_date, end_date)
    elif selected_page == "🗑️ Data Management":
        show_data_management_page()
    elif selected_page == "⚙️ Settings":
        show_settings_page()
    elif selected_page == "🛠️ User Management":
