...
import sqlite3
import io
from datetime import datetime # Ensure datetime from datetime is imported
...
def json_serializer_default(obj):
    # Handle pandas Timestamp and Python datetime objects (pd.Timestamp is a subclass of datetime.datetime)
    if isinstance(obj, datetime): # This will catch pd.Timestamp, datetime.datetime, datetime.date
        if hasattr(obj, 'tzinfo') and obj.tzinfo is not None and obj.tzinfo.utcoffset(obj) is not None:
             # If timezone-aware, convert to UTC and then to ISO format
            # Check for NaT before conversion
            if isinstance(obj, pd.Timestamp) and pd.isna(obj): # Handle pd.NaT
                 return None
            return obj.astimezone(pd.timezone('UTC')).isoformat()
        else:
            # For naive datetime or if already UTC (or if conversion not desired/problematic)
            # Check for NaT before isoformat
            if isinstance(obj, pd.Timestamp) and pd.isna(obj): # Handle pd.NaT
                 return None
            # datetime.date also has isoformat()
            return obj.isoformat()

    # Handle numpy datetime64 objects
    elif isinstance(obj, np.datetime64):
        if np.isnat(obj):  # Handle numpy NaT
            return None
        # Convert to pandas Timestamp to correctly get ISO format, handles units
    ...
    # Handle numpy number types
        ...
        # Handle NaN, Inf, -Inf for floats
        if np.isnan(obj) or np.isinf(obj):
            return str(obj) # Or None, or a specific string like "NaN" / "Infinity"
        return float(obj)
    elif isinstance(obj, np.bool_):
        return bool(obj)
    
    # Handle numpy arrays: convert to list, elements will be recursively serialized
    elif isinstance(obj, np.ndarray):
        return obj.tolist()
    
    # Handle pandas Series: convert to dict, elements will be recursively serialized
    elif isinstance(obj, pd.Series):
        return obj.to_dict() # Note: if series contains complex types, they also need handling
    
    # Handle other specific pandas objects by converting to string
    elif isinstance(obj, (pd.Interval, pd.Period, pd.Categorical)):
        return str(obj)
    
    # Fallback for any other object that might have an isoformat method (e.g., other date/time libraries)
    # Check this before the final str() fallback for more graceful serialization if possible.
    if hasattr(obj, 'isoformat') and callable(getattr(obj, 'isoformat')):
        try:
            return obj.isoformat()
        except Exception:
            # If isoformat() exists but fails (e.g., requires arguments or is not for serialization)
            pass # Fall through to the final str() conversion

    # Ultimate fallback: convert to string.
    # This should handle most other cases. If str(obj) itself raises an error,
    # it indicates a deeper issue with the object's string representation.
    try:
        return str(obj)
    except Exception:
        # If even str(obj) fails, return a placeholder string to prevent total serialization failure.
...
<style>
    ...
    .css-1d391kg .css-1v0mbm { /* Sidebar text color */
        color: white;
    }
    .stButton > button {
        background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
...
class DatabaseManager:
        ...
        try:
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS exceptions (
                    ...
                    original_row_data TEXT, -- Stores the full original row as JSON
            ...
            if 'original_row_data' not in columns: # This check was inside the original file already
    ...
    def save_exceptions(self, run_id, exceptions_df):
        ...
        try:
            ...
            for _, row_series in exceptions_df.iterrows():
                ...
                data_to_insert.append(
                    (run_id,
                     ...
                     serialized_row_data # Use the result of our robust serialization
                    )
                )
        ...
        except Exception as ex: # Catch other unexpected errors during the process
            ...
            if conn and getattr(conn, 'in_transaction', False): conn.rollback() # Check if in_transaction exists
            # Do not re-raise generic Exception 'ex' here if it's a data issue we logged,
            # unless it's critical to stop the whole flow. The loop continues for other rows.
            # If 'ex' is a TypeError from json.dumps itself (despite the robust serializer),
            # it implies a very fundamental issue, but our serializer should prevent that.
        finally:
            if conn: conn.close()

    def save_user_performance(self, run_id, df, exceptions_df):
        # ... (keep existing save_user_performance method as is)
        conn = None
        try:
            ...
            # Ensure 'Created user' is string and stripped for consistent grouping/merging
            ...
            # Prepare user_stats based on exceptions_df
            if exceptions_df.empty or 'Created user' not in exceptions_df.columns:
                ...
                # Filter for valid users and add those with no exceptions
            ...
            if user_stats.empty: return # No user stats to save
            ...
            if conn: conn.rollback()
            raise
        except Exception as ex_gen: # Catch other potential errors (e.g. in DataFrame ops)
    ...
    def get_validation_history(self):
        # ... (keep existing get_validation_history method as is)
        conn = None
        try:
            conn = sqlite3.connect(self.db_path, timeout=DB_TIMEOUT)
    ...
    def get_exceptions_by_run(self, run_id):
        """Retrieve exceptions for a specific run, including all original columns."""
        conn = None
        all_records = []
        # This set will store all unique column names found across all JSON objects for this run.
        all_json_keys_for_run = set()
        try:
            conn = sqlite3.connect(self.db_path, timeout=DB_TIMEOUT)
            # Fetch all columns, including original_row_data
            ...
            if not raw_exceptions_df.empty:
                for _, db_row in raw_exceptions_df.iterrows():
                    # Initialize record_data with data from the parsed JSON first
                    ...
                    # Explicitly set/override critical fields from the database's direct columns.
...
class DataValidator:
    def __init__(self, base_ref_path="reference_data"):
        ...
        # Define sets for departments with specific validation exemptions
        self.no_crop_check = {
        ...
        self.no_activity_check.update({"Production", "Processing", "Parent Seed"}) # .update() for sets
        
        # Load reference data upon initialization
        self.ref_files = self._load_reference_data()
        
        # Training map for UI hints (remains from original dashboard logic)
        self.training_map = {
            ...
            'Incorrect FC-Vertical Name': 'Ensure Valid FC-Vertical Name.', # More generic
...
def process_uploaded_file(uploaded_file):
    """Processes an uploaded Excel file, validates it, and stores results."""
    try:
        df = pd.DataFrame() 
...
def show_analytics_page():
    ...
    if not validation_history.empty and 'id' in validation_history.columns:
        ...
        if not dept_summary_analytics_df.empty:
            ...
            fig_dept_analysis = px.bar(dept_summary_analytics_df.sort_values(by='exception_rate', ascending=False), x='department', y='exception_rate', title="Exception Rate by Department", labels={'exception_rate': 'Exception Rate (%)', 'department': 'Department'}, color='exception_rate', color_continuous_scale=px.colors.sequential.Reds_r, text_auto='.2f')
            fig_dept_analysis.update_layout(xaxis_title="Department", yaxis_title="Exception Rate (%)", margin=dict(l=20, r=20, t=60, b=100), xaxis_tickangle=-45)
...
def show_trends_page():
    ...
    fig_trends.add_trace(go.Scatter(x=trends_history_df['upload_time'], y=trends_history_df['total_exceptions'], mode='lines+markers', name='Total Exceptions', line=dict(color='#e53e3e', width=2), marker=dict(symbol="circle")))
    fig_trends.add_trace(go.Scatter(x=trends_history_df['upload_time'], y=trends_history_df['total_records'], mode='lines+markers', name='Total Records Processed', line=dict(color='#3182ce', width=2), marker=dict(symbol="square")))
    fig_trends.update_layout(title="Validation Trends Over Time", xaxis_title="Upload Date", yaxis_title="Count", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), margin=dict(l=20, r=20, t=60, b=20), hovermode="x unified")
    ...
    try:
        ...
        if selected_run_display_text:
            ...
            if 'Severity' in exceptions_for_run_df.columns and not exceptions_for_run_df['Severity'].dropna().empty:
                ...
                fig_severity_dist = px.bar(severity_value_counts.sort_values(by='Severity'), x='Severity', y='Count', title=f'Exception Severity Distribution for Run ID {selected_run_id}', color='Severity', color_continuous_scale=px.colors.sequential.OrRd, text_auto=True)
                fig_severity_dist.update_layout(xaxis_title="Severity Score", yaxis_title="Number of Exceptions")
...
def show_user_location_page():
    ...
    for col_ul in ul_required_cols:
        if col_ul not in ul_exceptions_df.columns: st.warning(f"Missing required column '{col_ul}' in exceptions data for this run. Analysis might be incomplete or inaccurate."); ul_exceptions_df[col_ul] = pd.NA 
    ul_summary_df = ul_exceptions_df.groupby(['Created user', 'Location.Name'], dropna=False).agg(Total_Severity_Score=('Severity', 'sum'), Number_of_Exceptions=('Exception Reasons', lambda x: x.notna().sum())).reset_index().rename(columns={'Created user': 'User', 'Location.Name': 'Location'})
    ...
    if not ul_summary_df.empty:
        fig_ul_exc_count_chart = px.bar(ul_summary_df, x='User', y='Number_of_Exceptions', color='Location', barmode='group', title="Exceptions by User and Location", labels={'Number_of_Exceptions': 'Number of Exceptions'}, color_discrete_sequence=px.colors.qualitative.Set2)
        fig_ul_exc_count_chart.update_layout(xaxis_tickangle=-45, margin=dict(b=120)); st.plotly_chart(fig_ul_exc_count_chart, use_container_width=True)
        fig_ul_severity_chart = px.bar(ul_summary_df, x='User', y='Total_Severity_Score', color='Location', barmode='stack', title="Total Severity by User and Location (Stacked)", labels={'Total_Severity_Score': 'Total Severity Score'}, color_discrete_sequence=px.colors.qualitative.Pastel)
        fig_ul_severity_chart.update_layout(xaxis_tickangle=-45, margin=dict(b=120)); st.plotly_chart(fig_ul_severity_chart, use_container_width=True)
    ...
    if 'Exception Reasons' in ul_exceptions_df.columns and ul_exceptions_df['Exception Reasons'].notna().any():
        ...
        fig_ul_top_errors = px.bar(ul_error_types_df.head(10), x='Error Type', y='Count', title="Top 10 Error Types by Occurrence", color='Count', color_continuous_scale=px.colors.sequential.Viridis, text_auto=True)
        fig_ul_top_errors.update_layout(xaxis_tickangle=-45, margin=dict(b=150)); st.plotly_chart(fig_ul_top_errors, use_container_width=True)
...
def show_user_performance_page():
    ...
    if not user_exceptions_history_df.empty and 'exception_reason' in user_exceptions_history_df.columns and user_exceptions_history_df['exception_reason'].notna().any():
        ...
        with mistake_chart_cols[0]:
            fig_user_pie_mistakes = px.pie(user_mistake_counts_df.head(7), names='Mistake Type', values='Count', title="Top Mistake Types Distribution", color_discrete_sequence=px.colors.qualitative.Safe)
            fig_user_pie_mistakes.update_traces(textposition='inside', textinfo='percent+label'); fig_user_pie_mistakes.update_layout(showlegend=False, height=400); st.plotly_chart(fig_user_pie_mistakes, use_container_width=True)
        with mistake_chart_cols[1]:
            fig_user_bar_mistakes = px.bar(user_mistake_counts_df.head(10), x='Mistake Type', y='Count', title="Top 10 Mistake Counts", color='Count', color_continuous_scale=px.colors.sequential.Mint, text_auto=True)
            fig_user_bar_mistakes.update_layout(xaxis_tickangle=-45, height=400, margin=dict(b=150)); st.plotly_chart(fig_user_bar_mistakes, use_container_width=True)
    ...
    if not user_perf_summary_for_display_df.empty and 'upload_time' in user_perf_summary_for_display_df.columns and 'exception_rate' in user_perf_summary_for_display_df.columns:
        ...
        fig_user_exc_rate_trend.add_trace(go.Scatter(x=user_perf_summary_for_display_df['upload_time'], y=user_perf_summary_for_display_df['exception_rate'], mode='lines+markers', name='Exception Rate (%)', line=dict(color='#FF6347', width=2.5), marker=dict(size=7, symbol="diamond")))
        fig_user_exc_rate_trend.update_layout(title="User Exception Rate Trend", xaxis_title="Run Date", yaxis_title="Exception Rate (%)", yaxis_ticksuffix="%", margin=dict(l=40, r=20, t=60, b=20), hovermode="x"); st.plotly_chart(fig_user_exc_rate_trend, use_container_width=True)
...
if __name__ == "__main__":
    ...
    try:
        # Ensure the log directory exists if it's more complex than just a filename
